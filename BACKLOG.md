...
5. **Automatic expiry for rejected and forgotten memory.** Retention is durable
...
6. **Semantic memory retrieval.** Retrieval is lexical full-text search plus
...
7. **Workspace-shared lore.** Lore is persona-scoped, so an entry used by
...
8. **Whether Task Model roles may send conversation-derived text to OpenAI.**
...
19. **Installed acceptance for conversational image editing.** Delivered under
