...
generation preset record, its backfill, and preset-directed planning are
delivered too. See
...
that nothing in Phase 3 starts before item 1 exists: the scene record is what
...
1. **Scene contract.** The Task Model emits a typed scene record rather than
...
2. **Shortlist routing with deterministic fallback.** Source: ADR 0030.
...
3. **Routing tester in settings.** Deliberately temporary tooling, documented
...
4. **Starter presets and the bundle format.** Source: ADR 0030.
...
5. **Multi-pass presets.** A preset declares its stages, including an identity
...
6. **Persona Identity Spec, and comparison demoted to advisory.**
    Source: ADR 0031.
...
7. **Image library and ready-image serving.** The serving half of
...
8. **Settings consolidation.** This program must reduce settings surface, not
...
9. **Scene backlog and idea generation.** Propose scenes for a persona from
...
10. **Idle scheduler.** Produce backlog scenes on the background lane during
...
11. **Photo sets.** One idea, several frames sharing wardrobe, room, lighting,
...
12. **Preference weighting.** Deliberately simple and inspectable.
    Source: ADR 0030.
...
13. **Preset export and import.** Source: ADR 0030.
...
18. **Second Task Model adapter.** The structured-output contract has exactly
...
An important distinction, because it changes what can start early: items 21-23
are provider-neutral infrastructure and could be built against the existing
Kokoro path behind a flag. Only items 24-25 genuinely require the approved
listening decision. Step 24 cannot select a provider until that decision
...
25. **Repeatable provider evaluation** on latency, reliability, and blind
    ...
    evaluation that unblocks item 24 and deferred roadmap steps 10-13.
...
- Realtime and streaming TTS - no endpoint is advertised until step 24 lands.
...
- Preset discovery, ratings, or a shared registry. Item 13 delivers a file an
