"""Allow the scene-from-message task-model role.

Asking for a picture of a message sent the message straight to the image model,
wrapped in an English instruction it then tried to draw. Turning that passage
into a typed scene is a task-model job, and a task-model job needs a role.

The role list is a CHECK constraint, and SQLite cannot alter one in place, so
the tables are rebuilt exactly as 0029 rebuilt them. Existing profiles are
copied verbatim; no row's configuration changes, and the new role is seeded
lazily like every other.
"""
...
revision = "0038_scene_from_message_role"
down_revision = "0037_memory_embeddings"
branch_labels = None
depends_on = None
