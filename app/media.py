...
def user_safe_image_error(exc, provider="openai"):
    ...
    if "safety" in (detail or "").lower() or "safety_violations" in (detail or "").lower():
        return (
            "I couldn't generate that image because the request was flagged by safety filters. Please try a safer rewording.",
            detail,
            req_id,
        )
    if provider == "openai" and "Supported values are" in (detail or "") and "Invalid value" in (detail or ""):
        return (
            "That image size is not supported by OpenAI. Please choose 1024x1024, 1024x1536, 1536x1024, or auto.",
            detail,
            req_id,
        )
    if isinstance(exc, urllib.error.HTTPError):
        ...
        if provider in {"local", "local/automatic1111"}:
            if status in (401, 403):
                return (
                    "Image generation failed: Automatic1111 rejected authentication. Check the API auth string and --api-auth setup.",
                    detail,
                    req_id,
                )
            if status == 404:
                return (
                    "Automatic1111 API endpoint not found. Ensure webui is running with the --api flag.",
                    detail,
                    req_id,
                )
            if status == 422:
                return (
                    "Automatic1111 rejected one or more generation parameters. Check image size/steps/sampler/scheduler values.",
                    detail,
                    req_id,
                )
            if 500 <= status <= 599:
                return (
                    "Automatic1111 failed while generating the image. Please retry after the server is ready.",
                    detail,
                    req_id,
                )
        ...
        if provider == "local/comfyui":
            if status in (401, 403):
                return (
                    "Image generation failed: ComfyUI rejected authentication. Check your API auth settings.",
                    detail,
                    req_id,
                )
            if status == 404:
                return (
                    "ComfyUI route not found. Verify the ComfyUI server URL and that the server is running.",
                    detail,
                    req_id,
                )
            if status == 422:
                return (
                    "ComfyUI rejected one or more workflow parameters. Check model, sampler/scheduler, and additional JSON.",
                    detail,
                    req_id,
                )
            if 500 <= status <= 599:
                return (
                    "ComfyUI failed while generating the image. Please retry after the server is ready.",
                    detail,
                    req_id,
                )
        ...
        if status in (401, 403):
            return (
                "Image generation failed: OpenAI rejected the request (check API key and image model access).",
                detail,
                req_id,
            )
        if status == 429:
        ...
        if status in (400, 404):
            return (
                "OpenAI couldn't generate that image from the current request. Please revise the prompt and try again.",
                detail,
                req_id,
            )
        if 500 <= status <= 599:
    ...
    if isinstance(exc, urllib.error.URLError):
        if provider in {"local", "local/automatic1111"}:
            return (
                "Image generation is currently unavailable because Automatic1111 could not be reached. Check URL and that webui is running with --api.",
                detail,
                req_id,
            )
        if provider == "local/comfyui":
            return (
                "Image generation is currently unavailable because ComfyUI could not be reached. Check URL and that the ComfyUI server is running.",
                detail,
                req_id,
            )
        return (
            "Image generation is currently unavailable because the image provider could not be reached. Please try again in a moment.",
            detail,
            req_id,
        )
...
def user_safe_video_error(exc):
    ...
    if isinstance(exc, urllib.error.HTTPError):
        ...
        if status in (401, 403):
            return (
                "Video generation failed: OpenAI rejected the request (check API key and video model access).",
                detail,
                req_id,
            )
        if status == 429:
        ...
        if status in (400, 404, 422):
            ...
            return (
                "OpenAI couldn't generate that video from the current request. Please revise the prompt and try again.",
                detail,
                req_id,
            )
        if 500 <= status <= 599:
    ...
    if isinstance(exc, urllib.error.URLError):
        return (
            "Video generation is currently unavailable because the OpenAI provider could not be reached. Please try again in a moment.",
            detail,
            req_id,
        )
    if isinstance(exc, TimeoutError):
