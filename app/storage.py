...
class BackupStore:
    ...
    def prune_backup_snapshots(self, limit=None):
        ...
        if limit <= 0:
            return
        paths = [
            path for path in self.backup_dir.glob("*.zip")
            if path.is_file() and self.backup_path_for_name(path.name)
        ]
    ...
    def create_backup_snapshot(self, include_media=False):
        ...
        tmp_zip_handle = tempfile.NamedTemporaryFile(prefix=f".{name}.", suffix=".tmp", dir=self.backup_dir, delete=False)
        ...
        try:
            with tempfile.TemporaryDirectory(prefix=".snapshot-", dir=self.backup_dir) as tmp_dir:
                ...
                with zipfile.ZipFile(tmp_zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
                    ...
                    entry_count += 1
                    if self.settings_json.exists() and self.settings_json.is_file() and not self.settings_json.is_symlink():
